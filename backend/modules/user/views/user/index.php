					...
					<?php Pjax::begin(['id' => 'users']) ?>
						...
						GridView::widget([
							...
							['class' => 'yii\grid\ActionColumn',
							    'template' => '{update}{view}{approve}{disable}{wallet}',
							    'buttons' => [
								...
								'wallet' => function ($url, $model) {
									return Html::a('<span class="fa fa-money" style="padding-top: 0px;font-size: 18px;"></span>', $url, [
										    'title' => Yii::t('app', 'Wallet History'),
										    'class' => 'actions',
										    'target' => '_blank',
									]);
								},
								'approve' => function ($url, $model) {
									if ($model->status == 0) {
										return Html::a('<i class="fa fa-ban" aria-hidden="true" style="color:red;font-size: 15px;"></i>', $url, [
							    ...
							    'urlCreator' => function ($action, $model) {
								    ...
								    if ($action === 'wallet') {
									    $url = Url::to(['/user/user-wallet/index', 'id' => $model->id]);
									    return $url;
								    }
								    if ($action === 'approve') {
