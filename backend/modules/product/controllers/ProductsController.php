	...
	public function actionCreate($id = NULL) {
		...
		if ($model->load(Yii::$app->request->post()) && Yii::$app->SetValues->Attributes($model)) {
			if ($model->validate()) {
				...
				try {
					if ($model->save() && $this->Savespecifications($specfctns, $model) && $this->ProductImages($model, $id)) {
						$transaction->commit();
...
//						if (!empty($value)) {
	...
	function ProductImages($model, $id = null) {
