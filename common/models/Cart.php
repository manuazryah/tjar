...
//            [['session_id'], 'string', 'max' => 225],
        ...
        public static function Checkout($ship_address, $bill_address, $payment_type) {
                if (isset(Yii::$app->user->identity->id)) {
                        Cart::check_product();
                        $cart = Cart::find()->where(['user_id' => Yii::$app->user->identity->id])->all();
                        $orders = Cart::addOrder($cart, $ship_address, $bill_address, $payment_type);
...
//                $this->redirect('mycart');
        ...
        public static function addOrder($cart, $ship_address, $bill_address, $payment_type) {
                ...
                $model->shipment_amount = $shippinng_limit > $model->total_amount ? $ship_charge : Cart::shipping_charge($cart);
