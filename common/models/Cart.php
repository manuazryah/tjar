...
//                $this->redirect('mycart');
    ...
    public function reward_points($orders) {
        $order_details = OrderDetails::find()->where(['order_id' => $orders['order_id']])->all();
        foreach ($order_details as $order) {
            $product_vendor = ProductVendor::findOne($order->product_id);
            $product = Products::findOne($product_vendor->product_id);
            $user = User::findOne(Yii::$app->user->identity->id);
            $user->reward_points = $user->reward_points + $product->reward_points;
            $user->save();
        }
    }
