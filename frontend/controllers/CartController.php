    ...
    public function actionAddShipping() {
        if (yii::$app->request->isAjax) {
            ...
            if (isset(Yii::$app->request->post()['UserAddress']['billing'])) {
                $this->SaveTemp(1, Yii::$app->request->post()['UserAddress']['billing']);
                !empty($delivery) ? $this->SaveTemp(2, Yii::$app->request->post()['UserAddress']['billing']) : '';
//                   Yii::$app->session['temp_user2'] = Yii::$app->request->post()['UserAddress']['billing'] . '/' . $delivery;
                ...
                $this->SaveTemp(1, $address_id);
                !empty($delivery) ? $this->SaveTemp(2, $address_id) : '';
...
//                    Yii::$app->session['temp_user2'] = $address_id . '/' . $delivery;
    ...
    public function actionAddBilling() {
        if (yii::$app->request->isAjax) {
            if (isset(Yii::$app->request->post()['UserAddress']['billing'])) {
                $this->SaveTemp(2, Yii::$app->request->post()['UserAddress']['billing']);
                echo json_encode(array('msg' => 'success', 'id' => Yii::$app->request->post()['UserAddress']['billing']));
                exit;
            } else {
                $address_id = $this->adduseraddress();
                $this->SaveTemp(2, $address_id);
...
//                    $cart_content = $this->cart_table($contents);
                } else {
            ...
            if (isset($cart_id)) {
                ...
                if ($cart->save()) {
                    ...
                    if (!empty($cart_items)) {
                        if (count($cart_items) != Yii::$app->request->post()['count']) {
                            $this->redirect(array('cart/mycart'));
//                            $cart_content = $this->cart_table($cart_items);
                        }
//                        $cart_content = $this->cart_table($cart_items);
                        $subtotal = $this->total($cart_items);
                        $grandtotal = $this->net_amount($subtotal);
                    }
//                    echo $cart_content;exit;
                    echo json_encode(array('msg' => 'success', 'subtotal' => sprintf('%0.2f', $subtotal), 'grandtotal' => sprintf('%0.2f', $grandtotal)));
                } else {
...
//                    echo '<pre>';
//                    print_r($product_allready);
//                    echo 'aaa' . $msd->id . '//' . $msd->product_id;
                    $msd->delete();
