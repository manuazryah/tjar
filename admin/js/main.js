...
$(document).ready(function () {
        ...
        $('body').on('click', '.btn-search-tag', function (e) {
                e.preventDefault();
                var canonical = $('#searchtag-canonical_name').val();
                $.ajax({
                        url: homeUrl + 'product/search-tag/canonical',
                        type: "post",
                        data: {canonical: canonical},
                        success: function (data) {
                                var $data = JSON.parse(data);
                                if ($data.con === "2") {
                                        $('.field-searchtag-canonical_name').addClass('has-error');
                                        $('.field-searchtag-canonical_name .help-block').html('').append($data.error);
                                } else {
                                        $('#add-search-tag').submit();
                                }
                        }, error: function () {
