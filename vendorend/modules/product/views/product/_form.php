...
<div class="product-vendor-form">
    ...
    <?php
    $min = \common\models\ProductVendor::find()->where(['admin_status' => 2, 'product_id' => $product_model->id])->min('price');
    $max = \common\models\ProductVendor::find()->where(['admin_status' => 2, 'product_id' => $product_model->id])->max('price');
    ?>
    <span style="float: right;">Price Range : <?= $min ?> - <?= $max ?></span>
    <?= $form->field($model, 'price')->textInput(['maxlength' => true]) ?>
    <div class="col-md-12"style="padding:0px;">

        <span style="float: right;">Commission : <?= $product_model->commisson == '' ? 0 : $product_model->commisson ?> %</span>
    </div>
    ...
    <?php
    echo '<label class="control-label" for="productvendor-expiry" style="color:black">Expiry Date</label>';
